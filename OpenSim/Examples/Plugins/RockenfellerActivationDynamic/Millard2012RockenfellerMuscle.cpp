...
void Millard2012RockenfellerMuscle::constructProperties()
{
    ...
    constructProperty_minimum_activation(0.005);
...
void Millard2012RockenfellerMuscle::extendFinalizeFromProperties()
{
    ...
    if(!get_ignore_tendon_compliance() && !use_fiber_damping) {
        ...
        OPENSIM_THROW_IF_FRMOBJ(get_minimum_gamma() < 0.001,
            InvalidPropertyValue, getProperty_minimum_activation().getName(),
            "Minimum activation cannot be less than 0.001");

        OPENSIM_THROW_IF_FRMOBJ(getMinControl() < get_minimum_gamma(),
        ...
        OPENSIM_THROW_IF_FRMOBJ(get_minimum_gamma() < 0.0,
            InvalidPropertyValue, getProperty_minimum_activation().getName(),
        ...
        set_minimum_gamma(clamp(0, get_minimum_gamma(), 1));
    ...
    if (!get_ignore_activation_dynamics()) {
        ...
        actMdl.set_optimal_fiber_length(getOptimalFiberLength());
        try {
...
double aMinimumGamma, double aKuh0)
{
    ...
    setRoh0(aRoh0);
    setGammaC(aGammaC);
    setMinimumGamma(aMinimumGamma);
    setMinimumActivation(aKuh0);
}
...
extendAddToSystem(SimTK::MultibodySystem& system) const
{
    ...
    if(!get_ignore_activation_dynamics()) {
        addStateVariable(STATE_CALCIUM_CONCENTRATION_NAME);
    }
    if(!get_ignore_tendon_compliance()) {
...
extendSetPropertiesFromState(const SimTK::State& s)
{
    ...
    if(!get_ignore_activation_dynamics()) {
        setDefaultGamma(getStateVariableValue(s, STATE_CALCIUM_CONCENTRATION_NAME));
    }
    if(!get_ignore_tendon_compliance()) {
    ...
    computeStateVariableDerivatives(const SimTK::State& s) const
{
    ...
    if(!get_ignore_activation_dynamics()) {
        ...
        if (appliesForce(s) && !isActuationOverridden(s)) {
            gammadot = getCalciumDerivative(s);
        }
        setStateVariableDerivativeValue(s, STATE_CALCIUM_CONCENTRATION_NAME, gammadot);
    }
