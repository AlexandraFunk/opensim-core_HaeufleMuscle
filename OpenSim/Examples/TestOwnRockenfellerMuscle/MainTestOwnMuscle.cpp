 ...
 *  This program is a forward simulation of  one muscle pulling on a block.
 */
...
int main()
{
    ...
    try {
        ...
        Model osimModel;
        osimModel.setName("RockenfellerActivationDynamics");
        ...
        double maxIsometricForce = 1420, optimalFiberLength = 0.2127, 
               ...
               default_gamma = 0.01, time_constant_hatze = 11.3, nue = 3, 
               roh_0 = 5.27, gamma_C = 1.37, minimum_gamma = 0.005,
               Kuh0 = 0.005;

        // Rockenfeller muscle (Millard2012EquilibriumMuscle with Rockenfeller activation dynamics)
        Millard2012RockenfellerMuscle* Rockenfeller =
                new Millard2012RockenfellerMuscle("Rockenfeller",
            maxIsometricForce, optimalFiberLength, tendonSlackLength, 
            pennationAngle, time_constant_hatze, nue, roh_0, 
            gamma_C, minimum_gamma, Kuh0);



        // Define the path of the muscles
        Rockenfeller->addNewPathPoint("Rockenfeller-point1", ground, 
            Vec3(0.0, halfLength, -0.35));
        Rockenfeller->addNewPathPoint("Rockenfeller-point2", *block, 
            Vec3(0.0, halfLength, -halfLength));

        // Fiber length
        Rockenfeller->setDefaultFiberLength(optimalFiberLength);
        Rockenfeller->setDefaultGamma(default_gamma);

        // Add the two muscles (as forces) to the model
        osimModel.addForce(Rockenfeller);
        ...
        muscleController->prescribeControlForActuator("Rockenfeller", new Constant(1.0));
        ...
        STOFileAdapter_<double>::write(statesTable, 
                                      "RockenfellerActivationDynamics_states.sto");

        auto forcesTable = reporter->getForcesTable();
        STOFileAdapter_<double>::write(forcesTable, 
                                      "RockenfellerActivationDynamics_forces.sto");

        // Save the muscle analysis results
        IO::makeDir("MuscleAnalysisResults");
        muscAnalysis->printResults("RockenfellerActivationDynamics", "MuscleAnalysisResults");
        ...
        osimModel.print("RockenfellerActivationDynamics_model.osim");
    }
